...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch, batch in enumerate(batchs):
            ...
            ## np.pad() 로 바꿔보기 ?!
            nx_bat = np.zeros((batch_size, uc_data.max_sequence_length)) 
            ny_bat = np.zeros((batch_size, uc_data.max_sequence_length)) 
            nt_bat = np.zeros((batch_size, uc_data.max_sequence_length)) 
            ...
            feed = {
                ...
                transformer.targets: nt_bat, 
                transformer.target_seq_length: t_seq_len,
            ...
            if DEBUG :
                feed_opt={
                    ...
                    'target_seq_length':transformer.target_seq_length,
                ...
                step, summ, _loss, _, acc = sess.run([gstep, train_summary_merge, transformer.loss, train_op, transformer.accuracy ], feed_dict=feed)
            ...
            if cur_step % EVAL_EVERY == 0:
                ...
                for dev_ep, dev_batch in enumerate(dev_batchs):
                    ...
                    if dev_ep == 0:
                        ...
                        print('devset    {} -> {}'.format(input_word, get_umchar_str(results.get('predict')[0])))
                blen = dev_ep+1
